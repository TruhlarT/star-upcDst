...
void Init(){
    ...
    hAnalFlow = new TH1I("AnalFlow", "CutsFlow", kMax-1, 1, kMax);
    hAnalFlow2 = new TH1I("AnalFlow2", "CutsFlow for 4pi", kMax-1, 1, kMax);
...
void Make(){
    ...
    hAnalFlow->Fill(kAllEvents);
    hAnalFlow2->Fill(kAllEvents);
    Clear();
    ...
    hAnalFlow->Fill(kTrigger);
    hAnalFlow2->Fill(kTrigger);
...
// Loop over arms - check if have good-quality tracks, selecting branch combination
    ...
    // Define IDs of branches based on known ID of the arm
        ...
        && rpTrackIdVec_perSide[E].size()==1 && rpTrackIdVec_perSide[W].size()==1)
        {
            ...
            hAnalFlow->Fill(kRPinFiducial);
            hAnalFlow2->Fill(kRPinFiducial);
            ...
            if(fourPiState)
            {
                if(vertexId[2] != vertexId[3] || vertexId[2] != vertexId[1])
                    return;
                hAnalFlow2->Fill(kSameVertex);
                hAnalysisFlow2->Fill(kSameVrtx);
                FillPlots(1,i,1);
            }else
            {
                hAnalFlow->Fill(kSameVertex);
            ...
            if(fourPiState)
            {
                ...
                hAnalFlow2->Fill(kZeroCharge);
                FillPlots(2,i,1);
            } else
            {
                hAnalFlow->Fill(kZeroCharge);
                hAnalysisFlow->Fill(kTotCH0);
                FillPlots(2,i,0);      
            }

            if(vertexesZ[0] < 80 && vertexesZ[0] > -80)
            {
                if(fourPiState)  
                    hAnalFlow2->Fill(kZVertex);
                else 
                    hAnalFlow->Fill(kZVertex);
                if(NhitsFit[0] >=25 && NhitsFit[1] >= 25)
                {
                    if(fourPiState)  
                        hAnalFlow2->Fill(kHitsFit);
                    else 
                        hAnalFlow->Fill(kHitsFit);
                    if(NhitsDEdx[0] >= 15 && NhitsDEdx[1] >= 15)
                    {
                        if(fourPiState)  
                            hAnalFlow2->Fill(kHitsDEdx);
                        else 
                            hAnalFlow->Fill(kHitsDEdx);
                        if(DcaZ[0] < 1 && DcaZ[0] > -1 && DcaZ[1] < 1 && DcaZ[1] > -1)
                        {
                            if(fourPiState)  
                                hAnalFlow2->Fill(kDCAz);
                            else 
                                hAnalFlow->Fill(kDCAz);
                            if(DcaXY[0] < 1.5 && DcaXY[1] < 1.5)
                            {
                                if(fourPiState)  
                                    hAnalFlow2->Fill(kDCAxy);
                                else 
                                    hAnalFlow->Fill(kDCAxy);
                                if(Eta[0] > -0.7 && Eta[0] < 0.7 && Eta[1] > -0.7 && Eta[1] < 0.7)
                                {
                                    if(fourPiState)  
                                        hAnalFlow2->Fill(kEta);
                                    else 
                                        hAnalFlow->Fill(kEta);
                                    if(t[0] < -0.12 && t[1] < -0.12 && t[0] > -1.0  && t[1] > -1.0)
                                    {
                                        if(fourPiState)  
                                            hAnalFlow2->Fill(kMandelT);
                                        else 
                                            hAnalFlow->Fill(kMandelT);
                                                       
                                        if(missingPt <= 0.1)
                                        {
                                            if(fourPiState)  
                                                hAnalFlow2->Fill(kExclusive);
                                            else 
                                                hAnalFlow->Fill(kExclusive);
                                        }
                                    }
                                }
                            }
